...
class ResNet18(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        def lr_lambda(current_step: int):
            if current_step < self.cfg.lr_warmup_steps: # Warmup for lr_warmup_steps then decay or constant
                return float(current_step) / float(max(1, self.cfg.lr_warmup_steps)) # Warmup step by slowly increasing the LR
            elif self.cfg.wd_schedule_type == "cosine": # Cosine decay
                ...
                return decayed
            elif self.cfg.wd_schedule_type == "linear": # Linear decay
                decay_steps = self.cfg.max_train_steps - self.cfg.lr_warmup_steps
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(enable_progress_bar=True, devices=1, accelerator="gpu", max_steps=args.max_train_steps, logger=logger, fast_dev_run=args.test_run, callbacks=[lr_monitor])
